...
class EmployeeDownloadProgressScreen extends StatefulWidget {
  final String employeeName;
  final bool includeAiInsights;
  final bool includeSnapshots;

  const EmployeeDownloadProgressScreen({
    super.key,
    required this.employeeName,
    required this.includeAiInsights,
    required this.includeSnapshots,
  });

  @override
  State<EmployeeDownloadProgressScreen> createState() =>
      _EmployeeDownloadProgressScreenState();
}

class _EmployeeDownloadProgressScreenState
    extends State<EmployeeDownloadProgressScreen> {
  ...
  Future<void> _startPDFDownload() async {
    try {
      ...
      setState(() {
        _status = 'Generating ${widget.employeeName}\'s Report...';
        _progress = 0.2;
      });

      final exportResponse = await _aiApiService.exportEmployeeReportPdf(
        employeeName: widget.employeeName,
        includeAiInsights: widget.includeAiInsights,
        includeSnapshots: widget.includeSnapshots,
      );
      ...
      print('Employee Export API response: $exportResponse');
      ...
      print('Employee PDF Download Error: $e');
    }
  }
  ...
  Future<void> _savePDFToDevice(List<int> bytes) async {
    try {
      if (Platform.isAndroid) {
        ...
        final fileName =
            '${widget.employeeName.toLowerCase().replaceAll(' ', '_')}_report_$timestamp.pdf';
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                '${widget.employeeName} PDF saved to Documents folder',
              ),
      ...
      if (result == true) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                '${widget.employeeName} PDF saved to Downloads folder',
              ),
              backgroundColor: Colors.green,
              duration: const Duration(seconds: 3),
            ),
    ...
    if (directory != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${widget.employeeName} PDF saved to App Files folder',
            ),
            backgroundColor: Colors.green,
            duration: const Duration(seconds: 3),
          ),
