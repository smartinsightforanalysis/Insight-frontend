...
class AnalyticsDownloadProgressScreen extends StatefulWidget {
  final bool includeAiInsights;
  final bool includeSnapshots;

  const AnalyticsDownloadProgressScreen({
    super.key,
  ...
  @override
  State<AnalyticsDownloadProgressScreen> createState() =>
      _AnalyticsDownloadProgressScreenState();
}

class _AnalyticsDownloadProgressScreenState
    extends State<AnalyticsDownloadProgressScreen> {
  ...
  Future<void> _startPDFDownload() async {
    try {
      ...
      setState(() {
        _status = 'Generating Analytics Report...';
        _progress = 0.2;
      });

      final exportResponse = await _aiApiService.exportAnalyticsReportPdf(
      ...
      print('Analytics Export API response: $exportResponse');
      ...
      print('Analytics PDF Download Error: $e');
    }
  }
      ...
      if (Platform.isAndroid) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Analytics PDF saved to Documents folder'),
    ...
    try {
      ...
      if (result == true) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Analytics PDF saved to Downloads folder'),
              backgroundColor: Colors.green,
              duration: Duration(seconds: 3),
            ),
          );
  ...
  Future<void> _saveToAppDirectory(List<int> bytes) async {
    ...
    if (directory != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Analytics PDF saved to App Files folder'),
            backgroundColor: Colors.green,
            duration: Duration(seconds: 3),
          ),
        );
